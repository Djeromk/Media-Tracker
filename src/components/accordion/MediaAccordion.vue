...
const visibleCount = computed(() => {
  ...
  return props.userMedia.filter(item => item.media?.type === selectedFilter.value).length
})
...
const groupedByStatus = computed(() => {
  const groups: Record<MediaStatus, UserMedia[]> = {
    backlog: [], in_progress: [], completed: [], dropped: [],
  }
  props.userMedia.forEach(item => {
...
function visibleCountInGroup(items: UserMedia[]): number {
  ...
  return items.filter(item => item.media?.type === selectedFilter.value).length
}
</script>
  ...
  <div class="w-full space-y-6">
    ...
    <div v-if="visibleCount === 0" class="card-padded text-center py-14">
      <div class="w-14 h-14 mx-auto mb-4 rounded-full bg-(--gray-100) flex items-center justify-center">
        <component
          :is="filters.find(f => f.value === selectedFilter)?.icon ?? LayoutList"
          :size="24"
          class="text-(--gray-300)"
        />
      </div>
